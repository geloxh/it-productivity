...
exports.getWidgets = async (req, res) => {
    try {
        ...
        const [recentTickets, overdueTasksRaw, aging3, aging7, announcements, warrantyExpiring] = await Promise.all([
        ...
        res.json({ recentTickets, overdueByAssignee, aging: { gt3: aging3, gt7: aging7 }, announcements, warrantyExpiring });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
};
...
exports.getActivity = async (req, res) => {
    try {
        const logs = await AuditLog.find()
            ...
            .select('action entity user timeStamp status');
        res.json(logs);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
    ...
    try {
        ...
        res.json({
            ...
            avgResolutionHours: avgResolutionTime[0]?.avgMs
                ? Math.round(avgResolutionTime[0].avgMs / 3_600_000) : 0,
            openUnassigned
        });
    } catch (error) {
        res.status(500).json({ error: error.message });
...
exports.getProjectMetrics = async (req, res) => {
    try {
        const [byStatus, byPriority, overdue] = await Promise.all([
            ...
            Project.countDocuments({ endDate: { $lt: new Date() }, status: { $nin: ['Completed', 'Cancelled'] } })
        ]);
